...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$('#exeSqlBtn').click(function(){
                var html = "<div style='padding: 3px;'><textarea id='sql' name='sql' style='width: 95%; height: 420px;'/></div>";
                var submit = function (v, h, f) {
                    if (f.sql == '') {
                        $.jBox.tip("请输入SQL脚本。", 'error', { focusId: "sql" });
                        return false;
                    }

                    $.post('${ctxAdmin}/gen/genTable/executeSql', {'sql': f.sql}, function(data, textStatus, jqXHR){
						if(data && data.tag && data.tag == 'SUCCESS') {
                            alert('执行SQL成功，点击确定后将跳转至【业务表添加】!');
                            location.href = "${ctxAdmin}/gen/genTable/form";
						} else {
						    alert('执行SQL失败!');
						}
                ...
                $.jBox(html, { width: 640, title: "执行数据库脚本窗口", submit: submit });
			});
		});
		function page(n,s){
			if(n) $("#pageNo").val(n);
...
<body>
	<ul class="nav nav-tabs">
		...
		<shiro:hasPermission name="gen:genTable:edit">
			<li><a href="${ctxAdmin}/gen/genTable/form">业务表添加</a></li>
			<li><a style="cursor: pointer;" id="exeSqlBtn">执行SQL建表</a></li>
		</shiro:hasPermission>
	</ul>
